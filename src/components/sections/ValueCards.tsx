    ...
    <Section id="valor" eyebrow="Propuesta de valor" title="Lealtad, confianza, diferenciación e impacto">
      <div className="grid gap-5 sm:grid-cols-2">
        {cards.map((c, i) => (
          ...
          >
            <div className="pointer-events-none absolute -right-8 -top-8 h-32 w-32 rounded-full bg-gradient-to-br from-kunan-primary/15 to-kunan-accent/10 blur-2xl transition group-hover:opacity-100" />
            <c.icon className="relative h-10 w-10 text-kunan-primary" aria-hidden />
            <h3 className="relative mt-4 text-xl font-semibold text-kunan-900">{c.title}</h3>
            <p className="relative mt-2 text-slate-600">{c.body}</p>
          </motion.article>
        ))}
      </div>
    </Section>
